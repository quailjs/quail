...
var TableHeadersComponent = (function () {
  ...
  function getHeadersFromScope(cell, tableMap) {
    ...
    for (i = 0; i < width; i++) {
      headerCells = headerCells.add(scanHeaders(tableMap, coords.x + i, coords.y, 0, -1));
    }

    for (i = 0; i < height; i++) {
      headerCells = headerCells.add(scanHeaders(tableMap, coords.x, coords.y + i, -1, 0));
    }
    return headerCells;
  }
  ...
  function getHeadersFromGroups(cell, tableMap) {
    ...
    cell.closest('thead, tbody, tfoot').find('th[scope=rowgroup]').each(function () {
  ...
  $.fn.tableHeaders = function () {
    ...
    this.each(function () {
      ...
      if ($this.is('[headers]')) {
        ...
        headers = headers.add(getHeadersFromScope($this, map)).add(getHeadersFromGroups($this, map));
      }
    });
