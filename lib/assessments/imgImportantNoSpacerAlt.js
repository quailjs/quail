...
var ImgImportantNoSpacerAlt = function ImgImportantNoSpacerAlt(quail, test, Case) {
  test.get('$scope').find('img[alt]').each(function () {
    ...
    test.add(_case);
    if (quail.isUnreadable($(this).attr('alt').trim()) && $(this).attr('alt').length > 0 && width > 50 && height > 50) {
      _case.set({
        status: 'failed'
      });
    } else {
