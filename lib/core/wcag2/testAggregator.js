...
var TestAggregator = (function () {
  var pointerMap = {
    ...
    getPointer: function getPointer(elm) {
  ...
  function getCombinedAssertions(aggregator, tests) {
    ...
    eachTestCase(tests, function (test, testCase) {
      ...
      var assertion = assertions[elms.indexOf(testCase.get('element'))];
  ...
  function getStackedAssertions(aggregator, tests) {
    ...
    eachTestCase(tests, function (test, testCase) {
      ...
      var newResult = testCase.get('status');
      var getResultPriority = wcag2.EarlAssertion.getResultPriority;
      var assert = asserts[elms.indexOf(testCase.get('element'))];
  ...
  function TestAggregator(config, testDefinitions, subject) {
    ...
    this.tests = jQuery.map(this.tests, function (test) {
  ...
  TestAggregator.prototype.filterDataToTests = function (data) {
    var names = jQuery.map(this.tests, function (test) {
      return test.name;
    });
    var testData = [];
  ...
  TestAggregator.prototype.getResults = function (tests) {
    ...
    if (filteredTests.length === 1 || this.type === 'combined') {
      ...
      window.console.error('Unknown type for aggregator ' + this.id);
    }

    // Return a default assert if none was defined
    if (assertions) {
      if (assertions.length === 0) {
        assertion = new wcag2.EarlAssertion({
          testCase: this.id,
