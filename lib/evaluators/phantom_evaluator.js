...
page.onError = function (msg, trace) {
  console.log(JSON.stringify(['Error on the evaluated page', msg, trace], undefined, 2));
...
page.onResourceRequested = function (requestData, request) {
  ...
  if (blocked) {
    console.log(JSON.stringify(['BLOCKED (' + reason + ')', 'Requested (' + requestData.method + ')', requestData.url]));
    request.abort();
  } else {
    console.log(JSON.stringify(['Requested (' + requestData.method + ')', requestData.url]));
  }
};

page.onResourceReceived = function (response) {
  if (response.status) {
    console.log(JSON.stringify(['Received', response.status, response.url]));
  }
};

page.onResourceTimeout = function (error) {
  console.log(JSON.stringify(['Resource timeout', error.errorCode, // it'll probably be 408
...
page.onResourceError = function (error) {
  ...
  if (error.errorCode !== 301) {
    console.log(JSON.stringify(['Resource error', 'Error code: ' + error.errorCode, error.errorString, error.url], undefined, 2));
  }
};

// This is the last chance to catch catestrophic errors.
phantom.onError = function (msg, trace) {
  console.log(JSON.stringify(['Error in the phantom runner', msg, trace], undefined, 2));
};
...
var date = new Date();
var timestamp = [date.getFullYear(), ('0' + (date.getMonth() + 1)).slice(-2), ('0' + date.getDate()).slice(-2), '-', ('0' + date.getHours()).slice(-2), ('0' + date.getMinutes()).slice(-2), ('0' + date.getSeconds()).slice(-2), '-', date.getTime()].join('');
    ...
    try {
      page.evaluate(function (tests, size) {
        ...
        jQuery('html').quail({
          ...
          testComplete: function testComplete(event, test) {
            var name = test.get('name');
          ...
          testCollectionComplete: function testCollectionComplete() {
          ...
          successCriteriaEvaluated: function successCriteriaEvaluated(eventName, successCriteria) {
            ...
            var looper = function looper(index, _case) {
