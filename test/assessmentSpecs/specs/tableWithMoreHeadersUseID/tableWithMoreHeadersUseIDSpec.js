describe('assessment: tableWithMoreHeadersUseID', function () {
  ...
  before('load webdrivers and run evaluations with Quail', function () {
    return quailTestRunner.setup({
        url: 'http://localhost:9999/tableWithMoreHeadersUseID/tableWithMoreHeadersUseID.html',
        assessments: [
  ...
  it('should have correct key under the test results', function () {
    expect(quailResults.tests).to.include.keys('tableWithMoreHeadersUseID');
  });

  it('should return the proper assessment for the test', function () {
    var cases = quailResults.tests['tableWithMoreHeadersUseID'].cases;
