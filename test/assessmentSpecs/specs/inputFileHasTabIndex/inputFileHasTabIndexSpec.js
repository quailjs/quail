describe('assessment: inputFileHasTabIndex', function () {
  ...
  before('load webdrivers and run evaluations with Quail', function () {
    return quailTestRunner.setup({
        url: 'http://localhost:9999/inputFileHasTabIndex/inputFileHasTabIndex.html',
        assessments: [
  ...
  it('should have correct key under the test results', function () {
    expect(quailResults.tests).to.include.keys('inputFileHasTabIndex');
  });

  it('should return the proper assessment for the test', function () {
    var cases = quailResults.tests['inputFileHasTabIndex'].cases;
