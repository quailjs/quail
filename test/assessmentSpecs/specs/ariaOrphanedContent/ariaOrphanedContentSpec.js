describe('assessment: ariaOrphanedContent', function () {
  ...
  before('load webdrivers and run evaluations with Quail', function () {
    return quailTestRunner.setup({
        url: 'http://localhost:9999/ariaOrphanedContent/ariaOrphanedContent.html',
        assessments: [
          'ariaOrphanedContent'
        ]
      })
  ...
  it('should have correct key under the test results', function () {
    expect(quailResults.tests).to.include.keys('ariaOrphanedContent');
  });

  it('should return the proper assessment for the test', function () {
    var cases = quailResults.tests['ariaOrphanedContent'].cases;
