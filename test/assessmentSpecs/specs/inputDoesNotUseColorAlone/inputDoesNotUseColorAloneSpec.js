describe('assessment: inputDoesNotUseColorAlone', function () {
  ...
  describe('the non-match case', function () {
    before('load webdrivers and run evaluations with Quail', function () {
      return quailTestRunner.setup({
          url: 'http://localhost:9999/inputDoesNotUseColorAlone/inputDoesNotUseColorAlone-nomatch.html',
          assessments: [
        ...
        .spread(function (_client_, _assessments_, _quailResults_) {
          ...
          cases = quailResults.tests.inputDoesNotUseColorAlone.cases;
        });
    });
    ...
    it('should have correct key under the test results', function () {
      expect(quailResults.tests).to.include.keys('inputDoesNotUseColorAlone');
    });
  ...
  describe('the match case', function () {
    before('load webdrivers and run evaluations with Quail', function () {
      return quailTestRunner.setup({
          url: 'http://localhost:9999/inputDoesNotUseColorAlone/inputDoesNotUseColorAlone.html',
          assessments: [
            'inputDoesNotUseColorAlone'
          ]
        })
