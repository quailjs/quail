describe('assessment: closingTagsAreUsed', function () {
  ...
  before('load webdrivers and run evaluations with Quail', function () {
    return quailTestRunner.setup({
        url: 'http://localhost:9999/closingTagsAreUsed/closingTagsAreUsed.html',
        assessments: [
          'closingTagsAreUsed'
        ]
      })
  ...
  it('should have correct key under the test results', function () {
    expect(quailResults.tests).to.include.keys('closingTagsAreUsed');
  });

  it('should return the proper assessment for the test', function () {
    var cases = quailResults.tests['closingTagsAreUsed'].cases;
