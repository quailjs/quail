describe('assessment: tableSummaryDescribesTable', function () {
  ...
  describe('the non-match case', function () {
    before('load webdrivers and run evaluations with Quail', function () {
      return quailTestRunner.setup({
          url: 'http://localhost:9999/tableSummaryDescribesTable/tableSummaryDescribesTable-nomatch.html',
          assessments: [
        ...
        .spread(function (_client_, _assessments_, _quailResults_) {
          ...
          cases = quailResults.tests.tableSummaryDescribesTable.cases;
        });
    });
    ...
    it('should have correct key under the test results', function () {
      expect(quailResults.tests).to.include.keys('tableSummaryDescribesTable');
    });
  ...
  describe('the match case', function () {
    before('load webdrivers and run evaluations with Quail', function () {
      return quailTestRunner.setup({
          url: 'http://localhost:9999/tableSummaryDescribesTable/tableSummaryDescribesTable.html',
          assessments: [
            'tableSummaryDescribesTable'
          ]
        })
